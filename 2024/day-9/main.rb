    ...
    blocks.uniq.reverse_each do |file_id|
      ...
      (0...right_pointer).each do |left_pointer| 
        if blocks[left_pointer, file_id_count] == free_space_block
          blocks[left_pointer, file_id_count] = [file_id] * file_id_count
          blocks[right_pointer, file_id_count] = free_space_block
          break
        end
      end
    end
...
p DiskFragmenter.compute_filesystem_checksum_part_1(File.read('input.txt'))
p DiskFragmenter.compute_filesystem_checksum_part_2(File.read('input.txt'))
